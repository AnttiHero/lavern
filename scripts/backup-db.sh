...
#
# Usage:
#   ./scripts/backup-db.sh                          # Default: ./data/marble.db → ./backups/
#   ./scripts/backup-db.sh /path/to/marble.db       # Custom source path
...
#   0 3 * * * /opt/marble/scripts/backup-db.sh >> /var/log/marble-backup.log 2>&1

set -euo pipefail

# ── Configuration ────────────────────────────────────────────────────────

DB_PATH="${1:-./data/marble.db}"
BACKUP_DIR="${DB_BACKUP_DIR:-./backups}"
RETAIN_DAYS="${DB_BACKUP_RETAIN_DAYS:-30}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_FILE="${BACKUP_DIR}/marble_${TIMESTAMP}.db"
...
DELETED=$(find "$BACKUP_DIR" -name "marble_*.db" -mtime +"$RETAIN_DAYS" -delete -print | wc -l)
...
TOTAL=$(find "$BACKUP_DIR" -name "marble_*.db" | wc -l)
