 ...
 *
 * Marble supports two LLM providers:
...
/**
 * Model mapping: Marble cost tier → Mistral model.
 *
...
/** Reverse map: resolve a Marble tier model name to its Mistral equivalent. */
export function resolveModel(marbleModel: string, provider: LLMProvider): string {
  if (provider === 'anthropic') return marbleModel;

  // Map Claude model names → Mistral equivalents
  if (marbleModel.includes('opus')) return MISTRAL_MODELS.opus;
  if (marbleModel.includes('sonnet')) return MISTRAL_MODELS.sonnet;
  if (marbleModel.includes('haiku')) return MISTRAL_MODELS.haiku;
