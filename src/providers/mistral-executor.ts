...
export async function runMistralWorkflow(
  ...
  if (assessment.outcome !== 'completed') {
    ...
    logger.error('Engagement did not complete — no assembly', { workflow: template.id, provider: 'Mistral', outcome: assessment.outcome, reasons: assessment.reasons });
    session.events.emitEvent({
      type: 'error',
  ...
  // 'intake' after session_end (mirror of local-executor.ts).
